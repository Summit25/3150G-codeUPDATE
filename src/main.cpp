...
int BTask(void) {
  ...
  while(true) {
    ...
    if(BTaskActiv==1) {
      ...
      if(abs(LiftSensor.position(degrees)) < 200) {
        RunLift(100);
        if(abs(LiftSensor.position(degrees)) > 190) {
          BTaskActiv = 0;
        }
      } 
      else if(abs(LiftSensor.position(degrees)) > 190) {
        RunLift(-100);
        if(abs(LiftSensor.position(degrees)) <  200) {
          BTaskActiv = 0;
        }
    ...
    if(Controller1.ButtonB.pressing() && ButtonPressingB == 0) {
      ButtonPressingB=1;
      BTaskActiv=1;
    }
